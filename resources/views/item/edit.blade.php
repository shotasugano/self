...
<div class="panel-body">
    ...
    {{ csrf_field() }}
        ...
        <!-- アドレス削除ボタン -->
    <form action="{{ url('items/editapp/{id}') }}" method="POST" class="form-horizontal">
        {{ csrf_field() }}
        <input type="hidden" name="id" value="{{$item->id}}">
        <input type="hidden" name="name"  class="form-control" value= "削除">
        <input type="hidden" name="type"  class="form-control" value= 1>
        <input type="hidden" name="detail"  class="form-control" value= "削除">
        <div class="form-group">
            <div class="col-sm-offset-3 col-sm-6">
                <button type="submit" class="btn btn-default">
                    <i class="fa"></i> 削除申請
